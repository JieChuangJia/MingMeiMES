...
{
    /// <summary>
    /// 数据访问类:LOCAL_MES_STEP_INFOModel
    /// </summary>
    public partial class LOCAL_MES_STEP_INFODal
    {
        public LOCAL_MES_STEP_INFODal()
        { }
        #region  BasicMethod
        ...
        public bool Add(FTDataAccess.Model.LOCAL_MES_STEP_INFOModel model)
        {
            ...
            strSql.Append("RECID,SERIAL_NUMBER,STEP_NUMBER,CHECK_RESULT,STEP_MARK,STATUS,TRX_TIME,LAST_MODIFY_TIME,DEFECT_CODES,USER_NAME,REASON,UPLOAD_FLAG,AutoStationName)");
            ...
            SqlParameter[] parameters = {
					...
					new SqlParameter("@CHECK_RESULT", SqlDbType.Int,4),
					new SqlParameter("@STEP_MARK", SqlDbType.Int,4),
					new SqlParameter("@STATUS", SqlDbType.Int,4),
					new SqlParameter("@TRX_TIME", SqlDbType.DateTime),
					new SqlParameter("@LAST_MODIFY_TIME", SqlDbType.DateTime),
					new SqlParameter("@DEFECT_CODES", SqlDbType.NVarChar,255),
					...
					new SqlParameter("@AutoStationName", SqlDbType.NVarChar,50)};
            ...
            parameters[11].Value = model.UPLOAD_FLAG;
            parameters[12].Value = model.AutoStationName;
        ...
        public bool Update(FTDataAccess.Model.LOCAL_MES_STEP_INFOModel model)
        {
            ...
            parameters[10].Value = model.UPLOAD_FLAG;
            parameters[11].Value = model.AutoStationName;
            parameters[12].Value = model.RECID;
        ...
        public FTDataAccess.Model.LOCAL_MES_STEP_INFOModel GetModel(string RECID)
        {
            ...
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select  top 1 RECID,SERIAL_NUMBER,STEP_NUMBER,CHECK_RESULT,STEP_MARK,STATUS,TRX_TIME,LAST_MODIFY_TIME,DEFECT_CODES,USER_NAME,REASON,UPLOAD_FLAG,AutoStationName from LOCAL_MES_STEP_INFO ");
            strSql.Append(" where RECID=@RECID ");
            SqlParameter[] parameters = {
					new SqlParameter("@RECID", SqlDbType.NVarChar,255)			};
            parameters[0].Value = RECID;

            FTDataAccess.Model.LOCAL_MES_STEP_INFOModel model = new FTDataAccess.Model.LOCAL_MES_STEP_INFOModel();
        ...
        /// 得到一个对象实体
        /// </summary>
        public FTDataAccess.Model.LOCAL_MES_STEP_INFOModel DataRowToModel(DataRow row)
        {
            FTDataAccess.Model.LOCAL_MES_STEP_INFOModel model = new FTDataAccess.Model.LOCAL_MES_STEP_INFOModel();
            if (row != null)
            {
                ...
                {
                    model.CHECK_RESULT = int.Parse(row["CHECK_RESULT"].ToString());
                }
                if (row["STEP_MARK"] != null && row["STEP_MARK"].ToString() != "")
                {
                    model.STEP_MARK = int.Parse(row["STEP_MARK"].ToString());
                }
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select RECID,SERIAL_NUMBER,STEP_NUMBER,CHECK_RESULT,STEP_MARK,STATUS,TRX_TIME,LAST_MODIFY_TIME,DEFECT_CODES,USER_NAME,REASON,UPLOAD_FLAG,AutoStationName ");
        ...
        public DataSet GetListByPage(string strWhere, string orderby, int startIndex, int endIndex)
        {
            ...
            strSql.Append(")AS Row, T.*  from LOCAL_MES_STEP_INFO T ");
        ...
        public DataSet GetList(int PageSize,int PageIndex,string strWhere)
        {
            ...
            parameters[0].Value = "LOCAL_MES_STEP_INFO";
