...
namespace DevAccess
{
    ...
    /// </summary>
    public class PLCRwMCPro : IPlcRW
    {
        #region 数据 
       ...
       // private bool exitUdpRecv = false;
        ...
        #region 初始化
        public PLCRwMCPro(EnumPlcCata plcEnum,int db1Len, int db2Len)
        {
            this.plcCata = plcEnum;
            constantByte[0] = 0x50;//副标题
            constantByte[1] = 0x00;
            constantByte[2] = 0x00;//网络编号
            constantByte[3] = 0xFF;//PC编号
      ...
      //  public event EventHandler<LogEventArgs> eventLogDisp;
        ...
        private bool GetRecShortValues(byte[] recBuffer, int readAddrCount, ref short[] addrValues)
        {
            try
            {
                if (this.recBuffer.Count < recvLenRequire)
                {
                ...
                for (int i = 0; i < readAddrCount * 2; i += 2)
                {
                    ...
                    recValueList.Add(recBuffer[valueIndex + i + 1]);
            ...
            {
                Console.WriteLine("解析PLC返回数据发生异常"+ex.ToString());
                return false;
            }
        }
        /// <summary>
        ...
        /// 接收结果检查，
        /// </summary>
        /// <returns>0:正常，1：接收不全，2：PLC返回错误码</returns>
        ...
        private bool WaitRecvOK(int timeOut)
        {
            ...
            while(true)
            {
                ...
                {
                    lastErrorInfo = "PLC返回超时," + timeOut + "毫秒";
                    return false;
                }
            }
        }
